...
export class JsonRpcHistory extends IJsonRpcHistory {
  ...
  private storagePrefix = CHAT_CLIENT_STORAGE_PREFIX;
  ...
  public removeListener: IJsonRpcHistory["removeListener"] = (
    event,
    listener
  ) => {
  ...
  private async getJsonRpcRecords(): Promise<JsonRpcRecord[] | undefined> {
    const records = await this.core.storage.getItem<JsonRpcRecord[]>(
      this.storageKey
    );
    return records;
  }
